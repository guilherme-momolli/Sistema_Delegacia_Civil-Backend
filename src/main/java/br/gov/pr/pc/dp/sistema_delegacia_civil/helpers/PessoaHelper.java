...
public class PessoaHelper {
    ...
    public static void validarCpfAlterado(Pessoa pessoaExistente, PessoaRequestDTO dto, PessoaRepository pessoaRepository) {
        String cpfNovo = dto.getCpf();
        String cpfAtual = pessoaExistente.getCpf();

        if (cpfNovo != null && !cpfNovo.equals(cpfAtual)) {

            Optional<Pessoa> cpfExistente = pessoaRepository.findByCpf(cpfNovo);
            if (cpfExistente.isPresent() && !cpfExistente.get().getId().equals(pessoaExistente.getId())) {
                throw new IllegalArgumentException("O CPF informado já está cadastrado em outra pessoa.");
            }

            if (!validarCpf(cpfNovo)) {
